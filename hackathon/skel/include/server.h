 ...
 * @field log_fd: File descriptor for associated log file.
 * @field dim: Already written number of bytes
 * @field log_line_num: Number of log lines written
 */
struct lmc_cache {
	char *service_name;
	void *ptr;
...
int lmc_get_command(struct lmc_client *, int *);
...
int truncate_file_os(struct lmc_client *);
