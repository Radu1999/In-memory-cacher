...
lmc_send_stats(struct lmc_client *client)
{
	...
	if(snprintf(stats, 1000, LMC_STATS_FORMAT, time, client->cache->dim, (long unsigned int)client->cache->log_line_num) < 0) {
		return -1;
	}
	if(lmc_send(client->client_sock, stats, strlen(stats), LMC_SEND_FLAGS) == -1) {
		return -1;
	}
	return 0;
}
...
lmc_send_loglines(struct lmc_client *client)
{
	...
	char *token = strtok(client->cache->ptr, "\n");
	for(int i = 0; i < client->cache->log_line_num; i++) {
		if(lmc_send(client->client_sock, token, strlen(token), LMC_SEND_FLAGS) == -1) {
			return -1;
		}
		token = strtok(NULL, "\n");
	}
